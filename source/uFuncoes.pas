...
begin
     ...
     If (dm.tbFeriados.Locate('FER_DTFERI',M_DTFERI,[])) Then
         M_FLFERI := True;
     //
     If (DayOfWeek(M_DTMOVI) = 1) or (M_FLFERI) Then   // DOMINGO E FERIADOS
         If (M_HOMOVI >= M1_DOMFER) AND (M_HOMOVI <= M2_DOMFER) Then
            result := '2';
    If (DayOfWeek(M_DTMOVI) = 7 ) Then         // SABADO
       If (M_HOMOVI >= M1_SABADO) AND (M_HOMOVI <= M2_SABADO) OR (M_HOMOVI >= M3_SABADO) AND (M_HOMOVI <= M4_SABADO) Then
               result := '2';
   IF (M_HOMOVI >= M1_SEGSEX) AND (M_HOMOVI <= M2_SEGSEX) OR (M_HOMOVI >= M3_SEGSEX) AND (M_HOMOVI <= M4_SEGSEX) Then
          result := '2';
   //
   dm.tbFeriados.Close;
End;
